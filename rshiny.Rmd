...
output$adfDiff <- renderPrint({
  ...
  if (test_result$p.value < 0.05) {
    ...
    cat("⚠ Series may still not be stationary.\n")
  }
})
...
output$kpssDiff <- renderPrint({
  ...
  cat("Interpretation:\n")
  if (test_result$p.value < 0.05) {
    cat("⚠ Still not stationary – further differencing or transformation may be needed.\n")
  } else {
...
output$residualVerdict <- renderPrint({
 ...
 if (bp$p.value < 0.05) {
  ...
  if (issues == 0) {
    ...
    cat(paste("⚠", issues, "assumption(s) violated — consider model refinement.\n"))
  }
})
